...
|----------|------------------|---------|
| **[deploy-main.yml](./deploy-main.yml)** | Push to **`main`**, manual | `lint`, CI → SSH deploy → **verify live `/api/health`** (must match commit) |
| **[monitor-production.yml](./monitor-production.yml)** | Cron every **5 min**, manual | Public health check → SSH auto-recover if down |
| **[ci-debug.yml](./ci-debug.yml)** | Push/PR **`Debug`**, PR to **`main`** | `lint`, `npm test`, build (no deploy) |
| **[format-on-request.yml](./format-on-request.yml)** | Manual only | Prettier check or write — **not** part of standard CI |
...
**Branches:** **`Debug`** and **`main` only** — never create feature or `cursor/*` branches. See [docs/BRANCHING.md](../docs/BRANCHING.md).
