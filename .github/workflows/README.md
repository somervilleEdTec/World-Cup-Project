...
|----------|------------------|---------|
| **[deploy-main.yml](./deploy-main.yml)** | Push to **`main`**, manual | `format:check`, `lint`, CI → SSH deploy → **verify live `/api/health`** (must match commit) |
| **[monitor-production.yml](./monitor-production.yml)** | Cron every **5 min**, manual | Public health check → SSH auto-recover if down |
| **[ci-debug.yml](./ci-debug.yml)** | Push/PR **`Debug`**, PR to **`main`** | `format:check`, `lint`, `npm test`, build (no deploy) |
