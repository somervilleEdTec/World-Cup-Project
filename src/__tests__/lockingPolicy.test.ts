...
describe('locking policy (LOCKING.md)', () => {
  it('locks group-stage edits for a fixture with an official result before global kickoff', () => {
    const actual = { matchId: 'g-a-1', homeScore: 2, awayScore: 1 };
    expect(isMatchEditable(groupMatch, false, '2026-06-11T12:00:00Z', actual)).toBe(false);
  });
